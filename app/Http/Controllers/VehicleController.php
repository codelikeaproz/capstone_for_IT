...
class VehicleController extends Controller
{
    ...
    public function show(Vehicle $vehicle)
    {
        ...
        \Log::info('Vehicle Access Check', [
            'user_id' => Auth::id(),
            'user_municipality' => Auth::user()->municipality,
            'user_role' => Auth::user()->role,
            'vehicle_id' => $vehicle->id,
            'vehicle_municipality' => $vehicle->municipality,
            'can_access' => Auth::user()->canAccessMunicipality($vehicle->municipality),
        ]);
        ...
        if (!Auth::user()->canAccessMunicipality($vehicle->municipality)) {
            abort(403, 'You do not have permission to view this vehicle. User: ' . Auth::user()->municipality . ' | Vehicle: ' . $vehicle->municipality);
        }
